...
**Scope:** Production contract with implementation refinements through Milestone 13. Sections labeled with prior milestones record their scope at that time; the Milestone 9 persistence section supersedes the former in-memory-only completed-review lifetime. Milestone 13 adds explicit reports and PDF sharing.
...
### Reports and sharing (Milestone 13)

Both newly completed and historical `ScanReviewView` surfaces offer **View Report**, using one typed `AppRoute.report(CompletedScan)` and one `AccessibilityReportView`. The route carries the same immutable completed snapshot, including when automatic saving failed. It cannot access the camera, run analysis, change evidence, or save the scan again. Back returns to the originating review.

`Core/Reports` defines immutable, Sendable `AccessibilityReport`, `AccessibilityReportFinding`, `AccessibilityReportSection`, and text/style blocks. These are Foundation values, not SwiftUI, PDF, storage, or camera models. The format version is 1. The report is an explicit allowlist of readable content: scan completion date, summary/count, optional categorical scan quality, observations, evidence wording/rationale, valid estimates with uncertainty, possible impact, direct checks, optional improvements, and limitations. Scan/finding/session UUIDs, analyzer IDs, frame sequence/counts, normalized regions, raw model confidence, images, depth, and raw OCR streams are not report fields. Finalized recognized text that explains a finding is included and disclosed before export.

`AccessibilityReportGenerator` deterministically sorts supported findings by category (text contrast, then passage), evidence strength (strong, moderate, limited), then stable finding UUID as a tie breaker. IDs are used only during sorting and never rendered. Same scan, locale/time zone, and guidance rules produce equivalent report values; generation time/random IDs do not affect content. It reuses `AccessibilityGuidanceProviding` and the shared evidence-strength labels. Historical reports use current versioned guidance, not advice falsely attributed to the original capture. Zero findings explicitly do not confirm full accessibility. Optional legacy quality/evidence stays absent. The report mapper can also render the provider’s generic unsupported-category guidance without invented measurements or advice. Storage’s existing unknown-category rejection/quarantine is preserved: an unreadable future-schema scan is not silently coerced into a supported category, and requires a future supported migration before opening.

User-facing completion dates use a locale-aware `DateFormatter` with the current time zone. Passage evidence remains estimated, with RoomPlan/LiDAR method and quality; the separate measurement label uses Foundation’s locale-aware length format. The guidance observation retains its existing metre semantics. Strings are localized whole sentences or SwiftUI localizable keys, with common report headings/copy centralized in `ReportCopy`. A plain-text value uses the same blocks for interoperability/test comparison; there is no additional text-file sharing workflow in this milestone.

`AccessibilityReportPDFExporter` is a worker actor composed in `AppDependencies`. It invokes the pure generator for preview and the injected `ReportPDFRendering` boundary for export. `CoreTextReportPDFRenderer` uses native Core Text glyph layout and a Core Graphics file-backed PDF consumer. No screenshot/rasterized SwiftUI content is used. A4 pages have 48-point side/top margins, a separate footer zone, black text on white, hierarchical 23/17/12/11-point typography, and page numbers. Paragraphs continue from `CTFrameGetVisibleStringRange`’s UTF-16 offset; a heading reserves space for body lines. It rejects no-progress layout, unsupported format versions, over four million UTF-16 characters or over 1,000 pages. These are safety ceilings, not performance claims. Production scans remain bounded by existing 100-finding/8,192-character storage validation. PDF writing streams to disk rather than retaining page images or a full PDF `Data` buffer. The resulting file is reopened as a PDF before it is offered for sharing. Selectable text is verified; a tagged PDF/PDF-UA certification is not claimed.

The preview uses semantic headings, primary system colors/fonts, wrapping text, a scroll view, and 44-point action targets. The normal six-or-fewer findings use eager layout for stable accessibility headings; larger historical reports use lazy cards. Summary lines wrap independently. Back lives in an adaptive safe-area control strip rather than a fixed-height navigation-bar item, so it can grow with Dynamic Type while staying reachable during scrolling. It uses primary-color text and a bordered button. These choices address native audit findings for Dynamic Type, clipping and default-blue text contrast. The preview remains readable without generating a PDF. **Prepare PDF** moves `idle → generating → ready(URL)` or `failed(ReportError)`; failure retains the readable report and offers **Retry Export**. Only `ready` exposes native SwiftUI **Share Report** (`ShareLink`), supported by the iOS 17 deployment target. The user chooses Files/Mail/Messages/AirDrop or another installed compatible destination; AccessLens itself performs no upload. No automatic speech, haptics, overlay, color-only state or additional animation is introduced. Root Reduce Motion policy applies.

SwiftUI `.task(id:)` owns preview/export work; leaving cancels it. Actor work checks cancellation before preparation, during paragraph/page loops, and before publishing a file. MainActor owns only presentation state and rejects cancelled completions. No detached task, per-page task, unbounded work queue, raw frame or image buffer is introduced. Exporter actor isolation serializes rendering/file operations.

Temporary files live under the application temporary directory’s `AccessLensReportExports/<random UUID>/AccessLens-Report-yyyy-MM-dd.pdf`. The basename uses the scan date in UTC/Gregorian/POSIX form, contains no personal identifiers, and is deterministic; isolated directories prevent same-day collisions. Files request `completeUntilFirstUserAuthentication` protection. Failed/cancelled rendering immediately removes its own partial directory. Successful files survive report-screen dismissal to allow share extensions to finish reading; directories older than 24 hours are cleaned on the next export. Cleanup only considers immediate UUID directories, skips symbolic links/unrelated files, and never traverses a broad user directory. There is no permanent report store or database/schema change. Copies explicitly saved/shared by the user belong to their chosen destination and are outside app cleanup.

OSLog’s export category records preparation completion and classified failures without content, paths, identifiers or destinations. No networking, analytics, external AI, third-party library, images/depth exports, score or legal verdict is added. Apple API references: [ShareLink](https://developer.apple.com/documentation/SwiftUI/ShareLink), [Core Text](https://developer.apple.com/documentation/CoreText), and [visible string ranges](https://developer.apple.com/documentation/coretext/ctframegetvisiblestringrange(_:)).
...
Transient live state includes frame/sample-buffer references, raw Vision observations, in-flight work, analysis generation, quality signals, temporary overlays, and live stabilization history. Milestone 9 persists only completed scan metadata and finalized finding evidence through the repository. The navigation route holds an in-memory domain copy; persistence is independently owned. Milestone 13 adds transient reports and explicitly prepared temporary PDFs. Names and image retention remain future work. Raw frames, runtime queues, temporary overlays, candidates, and mutable live state are never persisted.
