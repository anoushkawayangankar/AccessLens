...
actor SwiftDataCompletedScanRepository: CompletedScanRepository {
    ...
    func fetchAll() throws -> [SavedScanSummary] {
        let context = try storage()
        do {
            let records = try context.fetch(FetchDescriptor<ScanStorageSchemaV3.StoredScan>())
            return records.map {
    ...
    private func record(id: UUID, context: ModelContext) throws -> ScanStorageSchemaV3.StoredScan? {
        var descriptor = FetchDescriptor<ScanStorageSchemaV3.StoredScan>(predicate: #Predicate { $0.id == id })
