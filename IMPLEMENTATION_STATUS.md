...
- Vision/OCR implementation: `VisionTextAnalyzer` performs real on-device `VNRecognizeTextRequest` work behind that scheduler. Results are normalized into text observations, conservatively classified as transient signage candidates, deduplicated in the scan feature, and shown with honest limitations.
- Visual contrast implementation: `VisualContrastAnalyzer` consumes only same-frame OCR regions tied to signage candidates, directly samples bounded in-memory BGRA crops, calculates estimated luminance/contrast evidence, and emits a transient potential-low-contrast candidate only after usable evidence and scan-local temporal confirmation. Contrast estimates are not compliance measurements. Other scene detection, final cross-category finding stabilization, persistence, and export remain unimplemented.
