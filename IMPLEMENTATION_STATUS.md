...
- Analysis foundation: camera callbacks reduce frames immediately to bounded metadata descriptors; a one-in-flight/one-pending scheduler owns transient session identity, replacement, cadence, cancellation, stale-result rejection, analyzer protocol, and typed partial-failure output. No actual analyzer is registered and no metadata output reaches UI or storage.
- Vision/OCR/detection/persistence/export production implementation: **not started**.
