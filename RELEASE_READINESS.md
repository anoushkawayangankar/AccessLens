...
|---|---|---|
| Build | DEBUG SCAN-COMPLETION BUILD PASSED | 2026-09-09 Debug `xcodebuild` passed against an iPhone 17 Pro Simulator (iOS 26.5). A clean Release build remains required. |
| Automated tests | FOUNDATION + CAMERA + OCR + CONTRAST + FINDING + REVIEW SUITES PASSED | `AccessLensTests`: 73 passed / 0 failed; `AccessLensUITests`: 10 passed / 0 failed on iPhone 17 Pro Simulator (iOS 26.5). Future focused and regression results remain required. |
...
| Privacy | ON-DEVICE TRANSIENT OCR + CONTRAST + FINDINGS + IN-MEMORY REVIEW; AUDIT PENDING | Camera frames are retained only while the one admitted analysis request or one latest pending request needs them; OCR, bounded pixel samples/crops, candidates, evidence summaries, findings, and the completed review snapshot are transient. The snapshot contains derived domain findings only and is not persisted, uploaded, or logged as content. No Photos/networking/accounts/analytics were added. Full privacy audit, retention disclosure, and future export consent/data preview remain required. |
